            ...
            1 => {
                ...
                if one_forward < 64 && board.board[one_forward] == EMPTY {
                    if square / 8 == promotion_rank {
                        moves.push(Move::with_promotion(square, one_forward, if current_sign == 1 { W_QUEEN } else { B_QUEEN }));
                        moves.push(Move::with_promotion(square, one_forward, if current_sign == 1 { W_KNIGHT } else { B_KNIGHT }));
                        moves.push(Move::with_promotion(square, one_forward, if current_sign == 1 { W_ROOK } else { B_ROOK }));
                        moves.push(Move::with_promotion(square, one_forward, if current_sign == 1 { W_BISHOP } else { B_BISHOP }));
                    } else {
                ...
                for &file_offset in &[-1, 1] {
                    ...
                    if target_square < 64 {
                        ...
                        if is_opponent || is_en_passant {
                            if square / 8 == promotion_rank {
                                moves.push(Move::with_promotion(square, target_square, if current_sign == 1 { W_QUEEN } else { B_QUEEN }));
                                moves.push(Move::with_promotion(square, target_square, if current_sign == 1 { W_KNIGHT } else { B_KNIGHT }));
                                moves.push(Move::with_promotion(square, target_square, if current_sign == 1 { W_ROOK } else { B_ROOK }));
                                moves.push(Move::with_promotion(square, target_square, if current_sign == 1 { W_BISHOP } else { B_BISHOP }));
                            } else {
...
fn make_move(board: &mut Board, mv: Move) -> MoveUndo {
    ...
    {
        let captured_pawn_square = if current_side == Color::White { mv.to + 8 } else { mv.to - 8 };
